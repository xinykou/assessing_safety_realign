...
python ./evaluation/poison/pred.py \
  ...
  --lora_folder /media/1/yx/model_merging_v2/saves/lora/baselines/clean_data/"${method_name}-${p_ratio}" \
	--instruction_path BeaverTails \
	--start 0 \
	--end 1000 \
	--output_path /media/1/yx/model_merging_v2/results/lora/baselines/clean_data/"${method_name}-${p_ratio}"/safety_generations.json


python ./evaluation/poison/eval_safety.py \
  --safety_evaluator_path ./pretrained_model/beaver-dam-7b \
  --input_path /media/1/yx/model_merging_v2/results/lora/baselines/clean_data/"${method_name}-${p_ratio}"/safety_generations.json
...
python evaluation/downstream_task/sst2_eval.py \
  ...
  --lora_folder /media/1/yx/model_merging_v2/saves/lora/baselines/clean_data/"${method_name}-${p_ratio}" \
	--start 0 \
	--end 1000 \
	--output_path /media/1/yx/model_merging_v2/results/lora/baselines/clean_data/"${method_name}-${p_ratio}"/downstream.json
