...
for tau in ${taus[@]}; do
    ...
    python ./evaluation/downstream_task/sst2_eval.py \
          ...
          --start 0 \
          --end 1000 \
          --output_path ./results/lora/realign/expo-mask_replace-safe_lora/${dataset_name}-${alignment_method}-${data_selected}-${region_method}/sparsity_ratio_${sparsity_ratio}-tau_${tau}-downstream.json \
          --add
