...
for sparsity_ratio in "${sparsity_ratios[@]}"; do
    ...
    for prune_rate in "${prune_rates[@]}"; do
        ...
        python ./evaluation/poison/pred.py \
            ...
            --start 0 \
            --end 1000 \
            --output_path ./results/lora/realign/adaptive_mask_replace-safe_lora/${dataset_name}-${alignment_method}-${data_selected}-${region_method}/sparsity_ratio_"${sparsity_ratio}"_prune_rate_"${prune_rate}"_epsilon_"${epsilon}"-safety.json

        # 将文件列表传递给 Python 脚本
#        python ./evaluation/poison/eval_safety.py \
#              --safety_evaluator_path ./pretrained_model/beaver-dam-7b \
#              --input_path ./results/lora/realign/adaptive_mask_replace-safe_lora/${dataset_name}-${alignment_method}-${data_selected}-${region_method}/sparsity_ratio_"${sparsity_ratio}"_prune_rate_"${prune_rate}"_epsilon_"${epsilon}"-safety.json
#              --add
