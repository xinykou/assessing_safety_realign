...
# 获取顶级目录
main_dir=$(dirname "$(dirname "$(dirname "$current_script_dir")")")  # ./
...
alignment_types=("expo_orpo_lora")  # "expo_sft_lora" "expo_kto_lora" "expo_simpo_lora"
fusion_effects=( "sft_to_orpo-alpha_0.9")  # "sft_to_sft-alpha_0.9" "sft_to_kto-alpha_0.9" "sft_to_simpo-alpha_0.9"

# Prune regions
for sparsity_ratio in "${sparsity_ratios[@]}"; do
    for i in "${!alignment_types[@]}"; do
