...
for sparsity_ratio in "${sparsity_ratios[@]}"; do
    ...
    for prune_rate in "${prune_rates[@]}"; do
        ...
        python ./evaluation/poison/eval_safety.py \
            ...
            --input_path ./results/lora/ablations/expo-adaptive_mask_replace-safe_lora/expo-adaptive_mask_replace-safe_lora/${dataset_name}-${alignment_method}-${data_selected}-${region_method}/sparsity_ratio_"${sparsity_ratio}"_prune_rate_"${prune_rate}"_epsilon_"${epsilon}"-safety.json \
            --add
