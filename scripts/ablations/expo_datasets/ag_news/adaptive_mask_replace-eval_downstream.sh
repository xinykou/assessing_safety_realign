...
for sparsity_ratio in ${sparsity_ratios[@]}; do
    ...
    for prune_rate in "${prune_rates[@]}"; do
        ...
        python ./evaluation/downstream_task/agnews_eval.py \
              ...
              --output_path ./results/lora/ablations/expo_datasets/adaptive_mask_replace-safe_lora/${dataset_name}-${alignment_method}-${data_selected}-${region_method}/sparsity_ratio_${sparsity_ratio}-prune_rate_${prune_rate}_epsilon_${epsilon}-downstream.json
