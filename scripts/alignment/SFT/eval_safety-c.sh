...
for checkpoint in 8000 16000 24000 32000 40000 48000
do
  echo "Evaluating safety checkpoint ${checkpoint}..."
CUDA_VISIBLE_DEVICES=5 python ./evaluation/poison/pred.py \
  --model_folder ${model_path} \
  --lora_folder ./saves/lora/sft-batch2/checkpoint-${checkpoint} \
	--instruction_path BeaverTails \
	--output_path ./results/lora/sft-batch2/safety_generations-checkpoint_${checkpoint}.json \

CUDA_VISIBLE_DEVICES=5 python ./evaluation/poison/eval_safety.py \
  --safety_evaluator_path /media/4/yx/model_cache/beaver-dam-7b \
  --input_path ./results/lora/sft-batch2/safety_generations-checkpoint_${checkpoint}.json
