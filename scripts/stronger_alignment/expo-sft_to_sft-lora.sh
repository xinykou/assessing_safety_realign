...
do
  ...
  CUDA_VISIBLE_DEVICES=1 python ./evaluation/poison/pred.py \
    ...
    --start 1000 \
    --end 1500 \
    --output_path ./results/lora/expo-${target_type}-lora/eval_dataset/${source_type}_to_${target_type}-alpha_${alpha}.json

  CUDA_VISIBLE_DEVICES=1 python ./evaluation/poison/eval_safety.py \
    --safety_evaluator_path ./pretrained_model/beaver-dam-7b \
    --input_path ./results/lora/expo-${target_type}-lora/eval_dataset/${source_type}_to_${target_type}-alpha_${alpha}.json \
    --add
